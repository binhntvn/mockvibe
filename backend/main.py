...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    user = db.execute(text(f"SELECT * FROM auth.users WHERE email = '{form_data.username}'")).first()
    if not user or not auth.verify_password(form_data.password, user.encrypted_password):
        raise HTTPException(
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    existing_user = db.execute(text(f"SELECT * FROM auth.users WHERE email = '{user.email}'")).first()
    if existing_user:
    ...
    db.execute(text(f"INSERT INTO auth.users (id, email, encrypted_password, role) VALUES ('{user_id}', '{user.email}', '{hashed_password}', 'authenticated')"))
    db.commit()
    ...
    created_user = db.query(models.Profile).filter(models.Profile.id == str(user_id)).first()
...
def create_order(order: schemas.OrderCreate, user_id: str = Depends(auth.get_current_user), db: Session = Depends(get_db)):
    db_order = models.Order(user_id=user_id, total_amount=order.total_amount, shipping_address=order.shipping_address)
    db.add(db_order)
    db.commit()
    db.refresh(db_order)
